...
import math #math
import msvcrt #for reacting to keystrokes
...
print "\
########################################################################################################################\n\
#\033[38;2;064;160;255mNOTICE: This program only searches for occurences of IDs in module files.                                             \033[0m#\n\
#\033[38;2;064;160;255mIt doesn't search for unused constant declarations, unused mission trigger definitions, or other unused special code. \033[0m#\n\
#\033[38;2;064;160;255mMake sure to clean them from your module.constants.py, module.mission_templates.py and other files.                   \033[0m#\n\
#\033[38;2;064;160;255mIf you don't, it will throw off the search, and you will get false positives - unused IDs counted as used.            \033[0m#\n\
#\033[38;2;064;160;255mAlso notice that objects that aren't called by ID name can still be called by ID number, either using math            \033[0m#\n\
#\033[38;2;064;160;255mto shift through a range of IDs, or by calling the ID by numerical value (in vanilla happens a lot for trp_player=0). \033[0m#\n\
#\033[38;2;064;160;255mIn other words, \"not found\" doesn't necessarily mean \"unused\", so double-check before removing something.             \033[0m#\n\
########################################################################################################################\n\
#\033[38;2;192;000;255mTIP: To quickly check just 1 type of objects, temporarily move the other ID files out of the module system.           \033[0m#\
########################################################################################################################\n"
#determine mode of operation
use_subfolders = False
if not os.path.exists(path+"\\module_info.py"): #if module_info.py not found in same folder as this file
	use_subfolders = True
if not os.path.exists(path+"\\module\\info.py") and use_subfolders: #if module\info.py not found in same folder as this file
	print "\033[38;2;255;000;000mERROR: no files to work on!\nPut this file in your module system.\033[0m"
	exit(1)
...
if use_subfolders:
	...
	id_match_prefix = ""
	temp_match_prefix = ""
	info_on_finish = "Check the files in the \\out folder for results."
else:
	module_path = path
	id_path = path
	temp_path = path
	...
	info_on_finish = "Check the out_*.py files for results."
#check if we won't overwrite something
detected_files = []
...
if os.path.exists(temp_path):
	for module_filename in os.listdir(module_path):
		...
		if module_matches and os.path.exists(temp_prefix+module_matches.group(1)+".py"):
			detected_files.append(temp_prefix+module_matches.group(1)+".py")
if os.path.exists(out_path):
	for id_filename in os.listdir(id_path):
		...
		if id_matches and os.path.exists(out_prefix+id_matches.group(1)+".py"):
			detected_files.append(out_prefix+id_matches.group(1)+".py")
l = (len(detected_files)==1) #this is for text formatting - True means use singular, False means use plural (I'm having fun with how lowercase l looks like 1 with Notepad++'s default font)
if len(detected_files)>0:
	if l: print "\033[38;2;255;255;000mWARNING: detected existing file with the same name as planned output of this program:"
	else: print "\033[38;2;255;255;000mWARNING: detected existing files with the same names as planned output of this program:"
	for detected_file in detected_files:
		print "* "+detected_file
	if l: print "If it's a leftover from a previous run of this program, it can be removed.\033[0m\n"
	else: print "If they are leftovers from a previous run of this program, they can be removed.\033[0m\n"
	if l: print "Remove the file? Choose [y]es or [n]o:",
	else: print "Remove the files? Choose [y]es or [n]o:",
	choice = ""
	while choice!="Y" and choice!="N":
		if msvcrt.kbhit(): choice = msvcrt.getch().upper()
	if choice=="N":
		print "No."
		if l: print "\nKeeping the file intact, aborting program."
		else: print "\nKeeping the files intact, aborting program."
		exit(0)
	else:
		print "Yes."
		for detected_file in detected_files:
			os.remove(detected_file)
		try: os.rmdir(temp_prefix) #also remove folder if empty
		except: pass
		if l: print "\nFile removed. Program will proceed."
		else: print "\nFiles removed. Program will proceed."
		print "\n------------------------------------------------------------------------------------------------------------------------\n"
...
if not os.path.exists(temp_path):
	os.makedirs(temp_path) #create folder for temp files if it doesn't exist
	temp_folder_created = True #and note that you did
for module_filename in os.listdir(module_path):
	#count % to display
	line_curr += 1
	...
	if module_matches:
		#open module file
		module_file = open(module_prefix+module_matches.group(1)+".py","r") #open the module file
		module_lines = module_file.readlines() #read its contents
		module_file.close() #close module file
		#write cleaned temp files (we want to remove everything that could throw off our search, like comments or strings containing #, and if anything else is removable that will make the file smaller, then why not remove that too)
		if os.path.exists(temp_prefix+module_matches.group(1)+".py"):
			print "\n\n\033[38;2;255;000;000mERROR: file "+temp_prefix+module_matches.group(1)+".py already exists! Program must be stopped.\033[0m" #user must have created the file while the program was running! (or deletion of the file failed)
			exit(1)
		temp_file = open(temp_prefix+module_matches.group(1)+".py","w") #open the temp file
		existing_temp_files.append(temp_prefix+module_matches.group(1)+".py")
		temp_prev = ""
...
print "\rCreating temp files to work on... \033[38;2;000;255;000mDONE.\033[0m"
print "\n------------------------------------------------------------------------------------------------------------------------\n"
#read ID files, and for each ID do a search in temp files for occurences
overwrite_prev_prnt = False
if not os.path.exists(out_path): os.makedirs(out_path) #create folder for out files if it doesn't exist
for id_filename in os.listdir(id_path):
	id_matches = re.match(id_match_prefix+"(.+)\.py",id_filename) #the contents of the (1st and only) capture group can be retrieved with id_matches.group(1)
	if id_matches:
		id_file = open(id_prefix+id_matches.group(1)+".py","r") #open the ID file
		id_lines = id_file.readlines() #read its contents
		id_file.close()
		#write out file with the results
		if os.path.exists(out_prefix+id_matches.group(1)+".py"):
			print "\n\033[38;2;255;000;000mERROR: file "+out_prefix+id_matches.group(1)+".py already exists! Program must be stopped.\033[0m" #user must have created the file while the program was running! (or deletion of the file failed)
			exit(1)
		out_file = open(out_prefix+id_matches.group(1)+".py","w") #open the out file
		if not overwrite_prev_prnt:
			print "Searching for IDs from "+id_prefix+id_filename+"... \033[38;2;255;000;000m0%\033[0m",
		else:
			print "\r                                                                                                                       ", #clear the line
			print "\rSearching for IDs from "+id_prefix+id_filename+"... \033[38;2;255;000;000m0%\033[0m",
		line_break = "" #no line break before 1st line
		line_curr = -1 #indexing from 0 for better math
		...
		for id_line in id_lines: #repeat for all lines
			...
			if last_int_percent<new_int_percent:
				last_int_percent = new_int_percent
				print "\rSearching for IDs from "+id_prefix+id_filename+"... \033[38;2;"+str(int(round(max(0,min(255.0*math.sqrt((100.0-last_int_percent)/50.0),255.0)))))+";"+str(int(round(max(0,min(255.0*math.sqrt(last_int_percent/50.0),255.0)))))+";000m"+str(last_int_percent)+"%\033[0m",
			...
			if id_matches.group(1)=="animations": comment = "\t#all animations are hardcoded by ID number, but not all are called by engine - unused ones can be repurposed"
			...
			#menus:
				...
				elif id_number==2:        comment = "\tused to be hardcoded by ID number for Custom Battle in classic M&B, but in Warband it's not - Custom Battle button in main menu directs to prsnt_game_custom_battle_designer instead"
			...
			#sounds:
				...
				if id_name in hardcoded_by_name: comment = "\t#hardcoded by ID name"
				if id_name=="snd_put_back_bow": comment = comment+" (when you sheathe your bow the engine doesn't play this sound, but requires it to exist anyway - most likely a bug)"
			#strings:
		...
		if os.path.getsize(out_prefix+id_matches.group(1)+".py")==0: #we can't refer to out_file variable, we have to refer to the actual file on the disk
			...
			overwrite_prev_prnt = False
			print "\rSearching for IDs from "+id_prefix+id_filename+"... \033[38;2;000;255;000mDONE.\033[0m"
if overwrite_prev_prnt:
	print "\r                                                                                                                       ", #clear the line
print "\n------------------------------------------------------------------------------------------------------------------------\n"
#remove temp files
print "Removing created temp files... \033[38;2;255;000;000m0%\033[0m",
for existing_temp_file in existing_temp_files: os.remove(existing_temp_file)
print "\rRemoving created temp files... \033[38;2;000;255;000mDONE.\033[0m"
#in subfolder mode also attempt to remove the temp folder
if use_subfolders and temp_folder_created:
	l = len(os.listdir(temp_path))==1
	if len(os.listdir(temp_path))>0:
		if l:
			print "\n\033[38;2;255;255;000mWARNING: 1 file remaining in "+temp_prefix+" folder:"
		else:
			print "\n\033[38;2;255;255;000mWARNING: "+str(len(os.listdir(temp_path)))+" files remaining in "+temp_prefix+" folder:"
		for temp_file in os.listdir(temp_path):
			print "* "+temp_file
		print "\033[0m\nRemove the whole "+temp_prefix+" folder? Choose [y]es or [n]o:",
		choice = ""
		while choice!="Y" and choice!="N":
			if msvcrt.kbhit(): choice = msvcrt.getch().upper()
		if choice=="N":
			print "No.\n"
			print "Leaving created "+temp_prefix+" folder intact."
		else:
			print "Yes.\n"
			print "Removing created "+temp_prefix+" folder... \033[38;2;255;000;000m0%\033[0m",
			shutil.rmtree(temp_prefix,ignore_errors=True)
			print "\rRemoving created "+temp_prefix+" folder... \033[38;2;000;255;000mDONE.\033[0m"
print "\n------------------------------------------------------------------------------------------------------------------------\n"
#show info on finish
print "\033[38;2;000;255;000mFINISHED.\033[0m "+info_on_finish